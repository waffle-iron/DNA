	...
	if completely {
		...
		*/
		for _, txVerify := range block.Transactions {
			...
			if err:= VerifyTransactionWithTxPool(txVerify,block.Transactions);err!=nil{
				return errors.New(fmt.Sprintf("VerifyTransactionWithTxPool failed when verifiy block"))
			}
		}
	}
